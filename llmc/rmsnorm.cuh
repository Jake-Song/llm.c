/*
rmsnorm CUDA kernel, and also Residual, because sometimes they are fused
...
__global__ void rmsnorm_forward_kernel3(floatX* __restrict__ out, float* __restrict__ mean, float* __restrict__ rstd,
...
__global__ void rmsnorm_forward_kernel6(floatX* __restrict__ out, float* __restrict__ mean, float* __restrict__ rstd,
                                    const floatX*  __restrict__ inp, const floatX*  __restrict__ weight,
                                    const floatX* __restrict__ bias, int N, int C) {
...
void rmsnorm_forward(floatX* out, float* mean, float* rstd,
    ...
    if (status == cudaSuccess) {
        rmsnorm_forward_kernel6<<<grid_size, dim3(WARP_SIZE, block_y), smem, stream>>>(out, mean, rstd, inp, weight, bias, N, C);
    } else {
        // fall back to the version without shared memory
        const int grid_size_fb = CEIL_DIV(N * WARP_SIZE, block_size);
        rmsnorm_forward_kernel3<<<grid_size_fb, block_size, 0, stream>>>(out, mean, rstd, inp, weight, bias, N, C);
    }
    cudaCheck(cudaGetLastError());
}
...
void fused_residual_forward5(floatX* residual, floatX* normed, float* mean, float* rstd,
    ...
    if(status == cudaSuccess) {
        ...
        rmsnorm_forward(normed, mean, rstd, residual, weight, bias, N, 1, C, stream);
    }
    cudaCheck(cudaGetLastError());
}

void rmsnorm_backward(floatX* dinp, floatX* dweight, floatX* dbias, float* scratch,
    ...
    rmsnorm_backward_kernel10<<<grid_size, block_size, shared_mem_size, stream>>>(dinp, dweight, dbias, scratch, dout, inp, weight, mean, rstd, B, T, C);
    cudaCheck(cudaGetLastError());
}
